...
// Test that flush() correctly rewinds the RAS after a misprediction
TEST_F(GenericPredictorTest, flush) {
  ...
  // Flush address (rewind RAS to address 36)
  predictor.flush(36);

  // Continue getting entries from RAS - should now skip the last popped entry
  prediction = predictor.predict(20, BranchType::Return, 0);
  EXPECT_TRUE(prediction.isTaken);
  EXPECT_EQ(prediction.target, 28);
