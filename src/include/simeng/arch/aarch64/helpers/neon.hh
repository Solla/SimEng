...
/** Helper function for NEON instructions with the format `fac<ge, gt> vd, vn,
 *  vm`.
 * T represents operand type (e.g. vd.2d is double).
 * C represents comparison type (e.g. for T=float, comparison type is
 * uint32_t).
...
RegisterValue vecFCompareAbs(srcValContainer& sourceValues,
  ...
  for (int i = 0; i < I; i++) {
    out[i] =
        func(std::fabs(n[i]), std::fabs(m[i])) ? static_cast<C>(-1) : 0;
  }
  return {out, 256};
}

/** Helper function for NEON instructions with the format `umaxv rd, vn.T`.
 * T represents the type of sourceValues (e.g. for vn.4h, T = uint16_t).
 * I represents the number of elements in the input vector (e.g. for vn.4h I =
 * 4).
 * Returns correctly formatted RegisterValue. */
template <typename T, int I>
